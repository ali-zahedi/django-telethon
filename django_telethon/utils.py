...
async def login_bot(client_session, bot_token, proxy):
    app = App.objects.first()
    if not app:
        raise ValueError("App does not exists.")
    telegram_client = TelegramClient(DjangoSession(client_session=client_session), app.api_id, app.api_hash, proxy=proxy)
...
async def login_user(client_session, phone_number, code, password, phone_code_hash, proxy):
    app = App.objects.first()
    if not app:
        raise ValueError("App does not exists.")
    telegram_client = TelegramClient(DjangoSession(client_session=client_session), app.api_id, app.api_hash, proxy=proxy)
...
async def send_code_request(client_session, phone_number, proxy):
    app = App.objects.first()
    if not app:
        raise ValueError("App does not exists.")
    telegram_client = TelegramClient(DjangoSession(client_session=client_session), app.api_id, app.api_hash, proxy=proxy)
...
async def connect_client(client_app, app, proxy):
    telegram_client = TelegramClient(DjangoSession(client_session=client_app), app.api_id, app.api_hash, proxy=proxy)
    await telegram_client.connect()
...
async def connect_clients(proxy):
    app = App.objects.first()
    if not app:
    ...
    for client in ClientSession.objects.filter(login_status__in=LoginStatus.approve()):
        await connect_client(client, app, proxy)


async def re_connect_clients(proxy):
    app = App.objects.first()
    if not app:
        logging.debug("App does not exists.")
        return
    ...
    for login in Login.objects.all().have_to_send_code():
        phone_hash = await send_code_request(login.client_session, login.phone_number, proxy)
    ...
    for login in Login.objects.all().have_to_login():
        if login.bot_token:
            login_result = await login_bot(login.client_session, login.bot_token, proxy)
        else:
            login_result = await login_user(
                login.client_session, login.phone_number, login.code, login.passcode, login.hash_code, proxy,
            )
        if login_result:
